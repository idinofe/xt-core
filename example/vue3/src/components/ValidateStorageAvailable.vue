...
const handleCheckLocalClick = async () => {
  ...
  await delay(1000)
  isLocalStorageAvaiable.value = validateStorageAvailable(StroageType.localStorage)
}

const handleCheckSessionClick = async () => {
  isSessionStorageAvaiable.value = '检测中'
  await delay(1000)
  isSessionStorageAvaiable.value = validateStorageAvailable(StroageType.sessionStorage)
}
